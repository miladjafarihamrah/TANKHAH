...
def generate_report(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            header_font = Font(bold=True, color="FFFFFF", size=14)  # فونت پررنگ، رنگ سفید و سایز ۱۴
            ...
            data_font = Font(size=14)  # سایز فونت ۱۴
            ...
            if report_type == 'expense' and filtered_data:
                ws.oddHeader.center.text = "گزارش ماهانه تنخواه"  # هدر وسط صفحه
                ws.append(['', '', 'جمع کل', total_amount, ''])
                # اعمال استایل به سطر جمع کل
                for cell in ws[ws.max_row]:
                    cell.font = Font(bold=True, size=14)  # سایز فونت ۱۴
                    cell.border = border
                    cell.alignment = alignment
            if report_type == 'mission' and filtered_data:
                ws.oddHeader.center.text = "گزارش ماهانه ماموریت"  # هدر وسط صفحه

            # اضافه کردن جای امضا در پایین صفحه
            ws.append([])  # یک سطر خالی
            ws.append([])  # یک سطر خالی
            ws.append([])  # یک سطر خالی
            ws.append([request.user.first_name, 'کنترل کننده', 'مدیر عامل'])  # جای امضای کاربر
